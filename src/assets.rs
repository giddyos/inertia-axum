...
impl ViteConfig {
    ...
    fn dev_runtime(&self, url: &str) -> Result<AssetRuntime, ConfigError> {
        ...
        if !matches!(uri.scheme_str(), Some("http" | "https")) || uri.authority().is_none() {
            return Err(ConfigError::new(format!("inertia-axum Vite configuration error\n\nVITE_DEV_SERVER_URL \"{url}\" must be an absolute http:// or https:// URL")));
        }
    ...
    fn production_runtime(&self) -> Result<AssetRuntime, ConfigError> {
        ...
        if !manifest.contains_key(&key) {
            let available = manifest
                ...
                .collect::<Vec<_>>()
                .join("\n");
            return Err(ConfigError::new(format!("inertia-axum Vite configuration error\n\nEntry \"{key}\" was not found in:\n{}\n\nAvailable entries:\n{available}", manifest_path.display())));
        }
        ...
        if !prefix.starts_with('/') || prefix.contains(['"', '<', '>', '&']) {
            return Err(ConfigError::new(format!("inertia-axum Vite configuration error\n\nPublic path \"{}\" must be an absolute, HTML-safe URL path", self.public_path)));
        }
