...
use axum::{
    http::{header::COOKIE, HeaderValue, Method, Uri},
    response::Response,
};
...
impl TransientSeed {
    pub(crate) fn capture<B>(request: &axum::http::Request<B>) -> Self {
        Self {
...
pub(crate) trait ErasedTransientStore: Send + Sync {
    ...
    fn commit<'a>(&'a self, response: &'a mut Response, data: TransientData)
        -> StoreFuture<'a, ()>;
}
