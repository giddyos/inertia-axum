...
impl<'request, 'metadata> SelectionPlan<'request, 'metadata> {
    ...
    pub(crate) fn includes(&self, prop: &str, mode: SelectionMode) -> bool {
        ...
        let explicitly_requested = self.partial_matches && context.partial_data.contains(prop);
        ...
        if self
            ...
            .once_props
            .iter()
            .any(|(key, once)| once.prop() == prop && context.except_once_props.contains(key))
            && !explicitly_requested
        {
            return false;
        }
        ...
        let included = if !self.partial_matches {
            true
        } else if !context.partial_except.is_empty() {
            !context.partial_except.contains(prop)
        } else if !context.partial_data.is_empty() {
            context.partial_data.contains(prop)
        } else {
            true
        };
